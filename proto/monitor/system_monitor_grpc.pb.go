...
const (
	SystemMonitor_Subscribe_FullMethodName = "/systemmonitor.SystemMonitor/Subscribe"
)
...
type SystemMonitorClient interface {
	Subscribe(ctx context.Context, in *SubscriptionRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SystemSnapshot], error)
}
...
func (c *systemMonitorClient) Subscribe(ctx context.Context, in *SubscriptionRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SystemSnapshot], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SystemMonitor_ServiceDesc.Streams[0], SystemMonitor_Subscribe_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[SubscriptionRequest, SystemSnapshot]{ClientStream: stream}
...
type SystemMonitor_SubscribeClient = grpc.ServerStreamingClient[SystemSnapshot]
...
type SystemMonitorServer interface {
	Subscribe(*SubscriptionRequest, grpc.ServerStreamingServer[SystemSnapshot]) error
...
func (UnimplementedSystemMonitorServer) Subscribe(*SubscriptionRequest, grpc.ServerStreamingServer[SystemSnapshot]) error {
...
func _SystemMonitor_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(SystemMonitorServer).Subscribe(m, &grpc.GenericServerStream[SubscriptionRequest, SystemSnapshot]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SystemMonitor_SubscribeServer = grpc.ServerStreamingServer[SystemSnapshot]
...
var SystemMonitor_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "systemmonitor.SystemMonitor",
	...
	Metadata: "monitor/system_monitor.proto",
}
